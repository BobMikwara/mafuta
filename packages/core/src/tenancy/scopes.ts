 ...
 * tanks, but were refused station writes and alert reads because `sites:write`
 * is not `stations:write`.
 *
 ...
 * rename are deliberately not added to legacy keys, because those keys were
 * never granted them. Migration `0004_rename_legacy_scopes` rewrites the stored
 * rows; this runtime mapping keeps a key working in the window between a deploy
 * and that migration.
 */
...
/** True when normalization would change the stored list. */
export function hasLegacyScopes(scopes: ReadonlyArray<string>): boolean {
  return scopes.some((scope) => isLegacyScope(scope));
}
